# ------------------------------------------------------------------------------
# Script used to test the percent that exhibit floor effect, where the est contains the
# 95% percentile above and below
# ------------------------------------------------------------------------------
...
run_trial <- function(trial_num, counter) {
  if (trial_num %% 100 == 0) {
  ...
  for (sampled_dex in 1:(length(sampled_ls) - 1)) {
    ...
    if (ncol(low_df) > 1 & ncol(high_df) > 1) {
      ...
      if (length(which(to_analyze$assignment_mech == 1)) < 1 | length(which(to_analyze$assignment_mech == 2)) < 1) {
        ...
        if (ordered_uniq_vals[length(ordered_uniq_vals)] < est) {
          inv_pval_above_est <- 1
        } else if (which(ordered_uniq_vals == est) == length(ordered_uniq_vals)) {
          inv_pval_above_est <- 1
        } else {
