...
def runSim(param_set):
    ...
    if I_first_half[-1] >= threshold:
        ...
        # Control
        surv_inf_con = dict.fromkeys(G.nodes(), days_intervention)
        for node in G.nodes():
        ...
        # Treatment
        surv_inf_trt = dict.fromkeys(G.nodes(), days_intervention)
        for node in G.nodes():
        ...
        with open(home + '/netVax/code_output/twostage/sims/2stg_N' + str(N_cluster) + "_k" + str(k_overdispersion) + "_R0wt" + str(R0_wt) + 
                  ...
                  '_vaxEff' + str(vax_eff) + '_assign' + str(assign) + '_sim' + str(sim_num) + '_SEIRS.csv', 'w') as out_f:
