...
def getPrelim(param_set):
    ...
    while (len(I_series) < nsim):
        ...
        for node in range(initial_infections_per_cluster):
            IC[node] = 'I'
        t, S, E, I, R, D, V, VE, VI = EoN.Gillespie_simple_contagion(G, H, J, IC, return_statuses, tmax = 200)
        next_t = 0
        to_add_row = []
    ...
    # Write output ------------------------------------------------------------    
    filename = home + "/netVax/code_output/prelim/N" + str(N_cluster) + "_k" + str(k_overdispersion) + "_R0" + str(R0) + "_eit" + str(eit) + '_NDV.csv'
