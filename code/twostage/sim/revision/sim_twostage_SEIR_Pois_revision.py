...
def runSim(param_set):
    ...
    for node in np.random.choice(G.nodes(), initial_infections_per_cluster, replace=False):
        IC[node] = 'I'
    ...
    with open(home + '/netVax/code_output/twostage/sims/2stg_N' + str(N_cluster) + "_R0wt" + str(R0_wt) + 
              ...
              '_vaxEff' + str(vax_eff) + '_assign' + str(assign) + '_assigntype' + str(assign_type) + '_sim' + str(sim_num) + '_SEIR_Pois_revision.csv', 'w') as out_f:
