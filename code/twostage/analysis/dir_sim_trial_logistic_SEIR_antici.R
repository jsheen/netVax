...
run_trial <- function(trial_num) {
  ...
  for (sampled_dex in 1:(length(sampled_ls) - 1)) {
    ...
    if (ncol(low_df) > 1 & ncol(high_df) > 1) {
      ...
      if (length(which(to_analyze$assignment_mech == 1)) < 101 | length(which(to_analyze$assignment_mech == 2)) < 101) { # make sure contrast error does not come up # Perhaps do something here if there is perfect separation of the outcomes
...
stopCluster(cl)
save(final, file = paste0("~/netVax/code_output/twostage/rData/final", R0_vax, "_SEIR_antici_dir.RData"))

# Load results -----------------------------------------------------------------
load(paste0("~/netVax/code_output/twostage/rData/final", R0_vax, "_SEIR_antici_dir.RData"))
