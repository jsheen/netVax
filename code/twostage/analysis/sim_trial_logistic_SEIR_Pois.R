  ...
  for (assignment_mech_dex in 1:length(assignment_mechanisms)) {
    ...
    for (trial_df_dex in seq(assignment_mech_dex, N_groups, length(assignment_mechanisms))) {
      ...
      if (cluster_df$node[1] == 'na') {
        ...
        mod_cluster_df$status <- ifelse(mod_cluster_df$time2inf_trt < cutoff, 1, 0) # Where 1 is infected, 0 is censored, and second part of condition is to discount the initial seeded
