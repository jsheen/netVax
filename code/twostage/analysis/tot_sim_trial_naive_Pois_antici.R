...
for (assignment_mechanism in assignment_mechanisms) {
  ...
  for (sim_num in 0:(N_sims - 1)) {
    test_cluster <- read.csv(paste0('~/netVax/code_output/twostage/sims/2stg_N1000_R0wt3_R0vax', R0_vax, '_eit0.005_vaxEff1_assign', assignment_mechanism, '_sim', sim_num, '_SEIR_Pois_antici.csv'))
...
# count = 0
# for (sim_num in 0:(N_sims - 1)) {
#   test_cluster <- read.csv(paste0('~/netVax/code_output/twostage/sims/2stg_N1000_k1_R0wt3_R0vax', R0_vax, '_eit0.005_vaxEff1_assign', 0, '_sim', sim_num, '_SEIR.csv'))
#   if (test_cluster$node[1] != 'na') {
#     if (length(which(test_cluster$time2inf_trt < 120)) < 10) {
#       count = count + 1
#     }
#   }
# }
...
run_trial <- function(trial_num) {
  ...
  for (realized_assign in rep(assignment_mechanisms, N_groups / length(assignment_mechanisms))) {
    cluster_for_trial <- read.csv(paste0('~/netVax/code_output/twostage/sims/2stg_N1000_R0wt3_R0vax', R0_vax, '_eit0.005_vaxEff1_assign', realized_assign, '_sim', clusters_to_use_final[clusters_to_use_dex], '_SEIR_Pois_antici.csv'))
  ...
  for (assignment_mech_dex in 1:length(assignment_mechanisms)) {
    ...
    for (trial_df_dex in seq(assignment_mech_dex, N_groups, length(assignment_mechanisms))) {
      ...
      if (cluster_df$node[1] == 'na') {
        ...
        if (length(which(cluster_df$assignment == 'e')) != 0) {
          mod_cluster_df <- cluster_df[which(cluster_df$assignment == 'e'),]
  ...
  for (sampled_dex in 1:(length(sampled_ls) - 1)) {
    ...
    if (ncol(low_df) > 1 & ncol(high_df) > 1) {
      ...
      for (clus_num_f in unique(to_analyze$clus_num)) {
        if (#length(which(to_analyze$assignment_mech[which(to_analyze$clus_num == clus_num_f)] == 2)) == 0 &
          #(length(unique(to_analyze[which(to_analyze$clus_num == clus_num_f),]$status)) == 1 | 
      ...
      if (length(which(to_analyze$assignment_mech == 1)) < 1 | length(which(to_analyze$assignment_mech == 2)) < 1) {
        ...
        # vals <- c()
        # for (f_clus_num in unique(to_analyze$clus_num)) {
        #   vals <- c(vals, length(which(to_analyze[which(to_analyze$clus_num == f_clus_num),]$status == 1)))
        # }
        
        # Permutation test for p-value
        outcomes <- to_analyze$status
        N_clus_c_perm <- as.integer(length(which(to_analyze$cond == 0)) / 100)
        N_clus_t_perm <- length(unique(to_analyze$clus_num[which(to_analyze$cond == 1)]))
        ...
        if (pval <= alpha) {
          ...
          for (iter in 1:num_bootstrap_sample) {
            ...
            while (!passed) {
              ...
              N_clus_treated <- length(unique(treated_to_analyze$clus_num))
...
stopCluster(cl)
save(final, file = paste0("~/netVax/code_output/twostage/rData/tot_final", R0_vax, "_naive_", assignment_mechanisms[2], "_", R0_vax, "_", N_assignment_mechanism_sets,"_Pois_antici.RData"))

# Load results -----------------------------------------------------------------
load(paste0("~/netVax/code_output/twostage/rData/tot_final", R0_vax, "_naive_", assignment_mechanisms[2], "_", R0_vax, "_", N_assignment_mechanism_sets,"_Pois_antici.RData"))
