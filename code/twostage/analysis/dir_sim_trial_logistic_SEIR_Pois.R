...
run_trial <- function(trial_num) {
  ...
  write.csv(c(clus_num_f_cnt / N_groups, (clus_num_f_cont_cnt / N_groups) / 2), paste0('~/netVax/scratch/', trial_num, '_poisreactionary.csv'))
...
stopCluster(cl)
save(final, file = paste0("~/netVax/code_output/twostage/rData/final", R0_vax, "_SEIR_Pois.RData"))

# Load results -----------------------------------------------------------------
load(paste0("~/netVax/code_output/twostage/rData/final", R0_vax, "_SEIR_Pois.RData"))
