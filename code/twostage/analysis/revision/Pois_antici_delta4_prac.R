  ...
  for (sampled_dex in 1:(length(sampled_ls) - 1)) {
    ...
    if (ncol(low_df) > 1 & ncol(high_df) > 1) {
      ...
      for (clus_num_f in unique(to_analyze$clus_num)) {
        if (length(which(to_analyze$clus_num == clus_num_f & to_analyze$status == 1)) < threshold_inclusion) {
...
stopCluster(cl)
#save(final, file = paste0("~/netVax/code_output/twostage/rData/final", R0_vax, "_naive_Pois_antici_", assignment_mechanisms[2], "_", N_assignment_mechanism_sets,"_delt4_revision_prac.RData"))

# Load results -----------------------------------------------------------------
#load(paste0("~/netVax/code_output/twostage/rData/final", R0_vax, "_naive_Pois_antici_", assignment_mechanisms[2], "_", N_assignment_mechanism_sets,"_delt4_revision_prac.RData"))
