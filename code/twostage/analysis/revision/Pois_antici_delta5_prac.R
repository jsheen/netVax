  ...
  for (assignment_mech_dex in 1:length(assignment_mechanisms)) {
    ...
    for (trial_df_dex in seq(assignment_mech_dex, N_groups, length(assignment_mechanisms))) {
      ...
      if (cluster_df$node[1] == 'na') {
        ...
        #mod_cluster_df <- mod_cluster_df[which(mod_cluster_df$assignment == 'ne_ni' | mod_cluster_df$assignment == 'ne_i'),]
        ...
        mod_cluster_df <- mod_cluster_df[which(mod_cluster_df$assignment == 'ne_ni'),]# Changed here
        ...
        if (pval <= alpha) {
          ...
          for (iter in 1:num_bootstrap_sample) {
            ...
            while (!passed) {
              ...
              N_clus_treated <- length(unique(treated_to_analyze$clus_num))  #N_clus_treated <- as.integer(nrow(treated_to_analyze) / 100)
...
stopCluster(cl)
save(final, file = paste0("~/netVax/code_output/twostage/rData/final", R0_vax, "_naive_Pois_antici_", assignment_mechanisms[2], "_", N_assignment_mechanism_sets,"_delt5_revision_prac.RData"))

# Load results -----------------------------------------------------------------
load(paste0("~/netVax/code_output/twostage/rData/final", R0_vax, "_naive_Pois_antici_", assignment_mechanisms[2], "_", N_assignment_mechanism_sets,"_delt5_revision_prac.RData"))
