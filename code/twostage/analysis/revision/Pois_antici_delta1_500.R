...
N_assignment_mechanism_sets = 14
...
for (assignment_mechanism in assignment_mechanisms) {
  assign_frac <- strsplit(assignment_mechanism, '_')[[1]][1]
  assign_type <- strsplit(assignment_mechanism, '_')[[1]][2]
  to_use <- c()
  for (sim_num in 0:(N_sims - 1)) {
    if (assign_frac == 0) {
      test_cluster <- read.csv(paste0('~/netVax/code_output/twostage/sims/2stg_N500_R0wt2_R0vax', 0, '_eit0.005_vaxEff', 0, '_assign', assign_frac, '_assigntype', assign_type, '_sim', sim_num, '_SEIR_Pois_antici_revision.csv'))
    } else {
      test_cluster <- read.csv(paste0('~/netVax/code_output/twostage/sims/2stg_N500_R0wt2_R0vax', R0_vax, '_eit0.005_vaxEff', vaxEff, '_assign', assign_frac, '_assigntype', assign_type, '_sim', sim_num, '_SEIR_Pois', reactionary_or_anticipatory, 'revision.csv'))
    }
...
run_trial <- function(trial_num) {
  if (trial_num %% 100 == 0) {
    write.csv(c('test'), paste0('~/netVax/scratch/', trial_num, '.csv'))
  }
  clusters_to_use <- c()
  ...
  for (realized_assign in rep(assignment_mechanisms, N_groups / length(assignment_mechanisms))) {
    ...
    if (assign_frac == 0) {
      cluster_for_trial <- read.csv(paste0('~/netVax/code_output/twostage/sims/2stg_N500_R0wt2_R0vax', 0, '_eit0.005_vaxEff', 0, '_assign', assign_frac, '_assigntype', assign_type, '_sim', clusters_to_use_final[clusters_to_use_dex], '_SEIR_Pois_antici_revision.csv'))
    } else {
      cluster_for_trial <- read.csv(paste0('~/netVax/code_output/twostage/sims/2stg_N500_R0wt2_R0vax', R0_vax, '_eit0.005_vaxEff', vaxEff, '_assign', assign_frac, '_assigntype', assign_type, '_sim', clusters_to_use_final[clusters_to_use_dex], '_SEIR_Pois', reactionary_or_anticipatory, 'revision.csv'))
    }
  ...
  for (assignment_mech_dex in 1:length(assignment_mechanisms)) {
    ...
    for (trial_df_dex in seq(assignment_mech_dex, N_groups, length(assignment_mechanisms))) {
      ...
      if (cluster_df$node[1] == 'na') {
        ...
        denom_to_add <- nrow(cluster_df) - 1
        num_to_add <- length(which(cluster_df$time2inf_trt < cutoff))
        ...
        # The following is prepare the dataframe for the analysis
  ...
  for (sampled_dex in 1:(length(sampled_ls) - 1)) {
    ...
    if (ncol(low_df) > 1 & ncol(high_df) > 1) {
      ...
      write.csv(c('err'), paste0('~/netVax/scratch/', trial_num, '.csv'))
      est_eff_res <- c(est_eff_res, NA)
      pval_res <- c(pval_res, NA)
    }
  }
  # Code to get true effect
...
if (assignment_mechanisms[1] == '0_trad') {
  save(final, file = paste0("~/netVax/code_output/twostage/rData/final", R0_vax, "_naive_Pois", reactionary_or_anticipatory, assignment_mechanisms[2], "_", N_assignment_mechanism_sets,"_delt1_revision_control_500.RData"))
} else {
  save(final, file = paste0("~/netVax/code_output/twostage/rData/final", R0_vax, "_naive_Pois", reactionary_or_anticipatory, assignment_mechanisms[2], "_", N_assignment_mechanism_sets,"_delt1_revision_500.RData"))
}

# Load results -----------------------------------------------------------------
if (assignment_mechanisms[1] == '0_trad') {
  load(paste0("~/netVax/code_output/twostage/rData/final", R0_vax, "_naive_Pois", reactionary_or_anticipatory, assignment_mechanisms[2], "_", N_assignment_mechanism_sets,"_delt1_revision_control_500.RData"))
} else {
  load(paste0("~/netVax/code_output/twostage/rData/final", R0_vax, "_naive_Pois", reactionary_or_anticipatory, assignment_mechanisms[2], "_", N_assignment_mechanism_sets,"_delt1_revision_500.RData"))
}
