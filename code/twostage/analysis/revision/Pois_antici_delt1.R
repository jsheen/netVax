...
run_trial <- function(trial_num) {
  ...
  for (realized_assign in rep(assignment_mechanisms, N_groups / length(assignment_mechanisms))) {
    ...
    cluster_for_trial <- read.csv(paste0('~/netVax/code_output/twostage/sims/2stg_N1000_R0wt2_R0vax', R0_vax, '_eit0.005_vaxEff', vaxEff, '_assign', assign_frac, '_assigntype', assign_type, '_sim', clusters_to_use_final[clusters_to_use_dex], '_SEIR_Pois_antici_new.csv'))
...
stopCluster(cl)
save(final, file = paste0("~/netVax/code_output/twostage/rData/final", R0_vax, "_naive_Pois_antici_", assignment_mechanisms[2], "_", N_assignment_mechanism_sets,".RData"))

# Load results -----------------------------------------------------------------
load(paste0("~/netVax/code_output/twostage/rData/final", R0_vax, "_naive_Pois_antici_", assignment_mechanisms[2], "_", N_assignment_mechanism_sets,".RData"))
