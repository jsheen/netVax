...
for (assignment_mechanism in assignment_mechanisms) {
  ...
  for (sim_num in 0:(N_sims - 1)) {
    test_cluster <- read.csv(paste0('~/netVax/code_output/twostage/sims/2stg_N1000_R0wt3_R0vax', R0_vax, '_eit0.005_vaxEff0.8_assign', assignment_mechanism, '_sim', sim_num, '_SEIR_Pois_antici.csv'))
...
run_trial <- function(trial_num) {
  ...
  for (realized_assign in rep(assignment_mechanisms, N_groups / length(assignment_mechanisms))) {
    cluster_for_trial <- read.csv(paste0('~/netVax/code_output/twostage/sims/2stg_N1000_R0wt3_R0vax', R0_vax, '_eit0.005_vaxEff0.8_assign', realized_assign, '_sim', clusters_to_use_final[clusters_to_use_dex], '_SEIR_Pois_antici.csv'))
    ...
    for (trial_df_dex in seq(assignment_mech_dex, N_groups, length(assignment_mechanisms))) {
      ...
      if (cluster_df$node[1] == 'na') {
        ...
        if (length(which(cluster_df$assignment == 'e')) > 0) {
          ...
          denom_to_add <- length(which(cluster_df$assignment == 'e'))
          num_to_add <- length(which(cluster_df$assignment == 'e' & cluster_df$time2inf_trt < cutoff))
          ...
          mod_cluster_df <- mod_cluster_df[sample(1:nrow(mod_cluster_df), N_sample),] # Sample some number of this, assuming samp_num << number that are 'e'
    ...
    if (ncol(low_df) > 1 & ncol(high_df) > 1) {
      ...
      if (length(which(to_analyze$assignment_mech == 1)) < 101 | length(which(to_analyze$assignment_mech == 2)) < 101) { # make sure contrast error does not come up
        ...
        if (summary(res.logistic)[10]$coefficients[8] < 0.05) {
          ...
          #est_eff_res <- c(est_eff_res, 1 - exp(summary(res.logistic)[10]$coefficients[2])) # This is to see coefficient effect
  ...
  write.csv(c(clus_num_f_cnt / N_groups, (clus_num_f_cont_cnt / N_groups) / 2), paste0('~/netVax/scratch/', trial_num, '_poisanticipatory.csv'))
...
stopCluster(cl)
save(final, file = paste0("~/netVax/code_output/twostage/rData/final", R0_vax, "_SEIR_Pois_antici.RData"))

# Load results -----------------------------------------------------------------
load(paste0("~/netVax/code_output/twostage/rData/final", R0_vax, "_SEIR_Pois_antici.RData"))
