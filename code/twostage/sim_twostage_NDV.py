...
def runSim(param_set):
    ...
    # Wild-type
    filename = home + "/netVax/code_output/prelim/N" + str(N_cluster) + "_k" + str(k_overdispersion) + "_R0" + str(R0_wt) + "_eit" + str(eit) + '_NDV.csv'
    ...
    R0_vax = vax
    if float(R0_vax) > 0:
        filename = home + "/netVax/code_output/prelim/N" + str(N_cluster) + "_k" + str(k_overdispersion) + "_R0" + str(R0_vax) + "_eit" + str(eit) + '_NDV.csv'
    ...
    H.add_edge('I', 'R', rate=gamma * (1 - mort))
    H.add_edge('I', 'D', rate=gamma * (mort))
    H.add_edge('V', 'S', rate=psi)
    H.add_edge('V', 'VE', rate=sigma)
    H.add_edge('VE', 'VI', rate=gamma)
    ...
    if I_first_half[-1] >= threshold:
        ...
        with open(home + '/netVax/code_output/twostage/sims/2stg_N' + str(N_cluster) + "_k" + str(k_overdispersion) + "_R0wt" + str(R0_wt) + 
                  ...
                  '_vaxEff' + str(vax_eff) + '_assign' + str(assign) + '_sim' + str(sim_num) + '_NDV.csv', 'w') as out_f:
