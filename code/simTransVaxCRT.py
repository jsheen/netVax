...
# -*- coding: utf-8 -*-
"""
Created on Thu Jun 30 13:08:18 2022

@author: jsheen

@description: script used to create cluster simulations for a CRT simulation.
              This is very similar in spirit to the NPI simulations done before.
"""
...
def runSim(param_set):
    ...
    if I_first_half[-1] >= threshold:
        ...
        enrolled_nodes = np.random.choice(suscep_nodes, size=int(np.ceil(0.5 * len(suscep_nodes))), replace=False)
        ...
        for node in G.nodes():
            ...
            if 'D' in node_hist[1]:
                ...
                if node_hist[1][0] == 'D':
                    surv_inf_trt[node] = -1
        # Write results
        with open(home + '/netVax_sim_results/CRT_N' + str(N_cluster) + "_k" + str(k_overdispersion) + "_R0wt" + str(R0_wt) + 
