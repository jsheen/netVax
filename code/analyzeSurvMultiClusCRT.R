...
# @author: Justin Sheen
#
# @description: script to analyze survival data of transmissible vaccine trial when it is a CRT
# ------------------------------------------------------------------------------
...
for (param_set in param_sets) {
  ...
  runSim <- function(N_cluster, overdispersion, R0_wt, R0_vax, mort, eit, vax_treat, vax_eff, sim_num, multi_cluster) {
    ...
    for (cluster_dex in 1:multi_cluster) {
      ...
      single_clus$clusCond <- ifelse(cluster_dex %% 2 == 0, 't', 'c')
      if (cluster_dex %% 2 == 0) {
        single_clus$time2inf <- single_clus$time2inf_trt
        single_clus$time2death <- single_clus$time2death_trt
      } else {
    ...
    res_cox_inf <- coxph(Surv(time2inf, status_inf) ~ clusCond + frailty(clusNum, distribution="gamma", sparse=FALSE), data = sim_res_infect)
    ...
    p_val_inf <- summary(res_cox_inf)[7]$coefficients[11]
    p_val_inf <- ifelse(is.na(p_val_inf), 1, p_val_inf)
    effect_estimate_inf <- 'na'
    if (p_val_inf <= 0.05) {
      effect_estimate_inf <- 1 - exp(unname(res_cox_inf$coefficients[1]))
    }
    # Death outcome
    sim_res_death$time2inf <- NULL
    sim_res_death$status_inf <- NULL
    res_cox_death <- coxph(Surv(time2death, status_death) ~ clusCond + frailty(clusNum, distribution="gamma", sparse=FALSE), data = sim_res_death)
    ...
    #            ggtheme = theme_minimal())
    p_val_death <- summary(res_cox_death)[7]$coefficients[11]
    p_val_death <- ifelse(is.na(p_val_death), 1, p_val_death)
    effect_estimate_death <- 'na'
    if (p_val_death <= 0.05) {
      effect_estimate_death <- 1 - exp(unname(res_cox_death$coefficients[1]))
    }
  ...
  finalMatrix <- foreach(i=seq(0, 4999, 1), .combine=rbind) %dopar% {
    library(survival)
    library(survminer)
    library(foreach)
    library(doParallel)
  ...
  power_inf <- length(which(as.numeric(finalMatrix_copy$p_val_inf) <= 0.05)) / 5000
  power_death <- length(which(as.numeric(finalMatrix_copy$p_val_death) <= 0.05)) / 5000
  ...
  colnames(new_row) <- c('N', 'k', 'R0_wt', 'R0_vax', 'mort', 'eit', 'vaxEff', 'multi_cluster',
...
for (row_dex in 1:nrow(res_df)) {
  ...
  l[[l_dex]] <- ggplot(hist_df, aes(x=effect_estimate)) + 
    ggtitle(paste0('Inf: R0_vax=', res_df$R0_vax[row_dex], '; Power=', round(res_df$power_inf[row_dex], digits=2), '; Clus=', res_df$multi_cluster[row_dex])) + 
  ...
  l2[[l2_dex]] <- ggplot(hist_df, aes(x=effect_estimate)) + 
    ggtitle(paste0('Death: R0_vax=', res_df$R0_vax[row_dex], '; Power=', round(res_df$power_death[row_dex], digits=2),'; Clus=', res_df$multi_cluster[row_dex])) + 
    geom_histogram(bins=30) + geom_vline(xintercept=0.6, col='red') + theme(plot.title = element_text(size = 5))
  l2_dex <- l2_dex + 1
}
ggsave(filename="~/netVax/code_output/plots/N=1000_inf_multi_CRT.pdf", marrangeGrob(grobs = l, nrow=1, ncol=3),
       width=9, height=3, units='in', dpi=600)
ggsave(filename="~/netVax/code_output/plots/N=1000_death_multi_CRT.pdf", marrangeGrob(grobs = l2, nrow=1, ncol=3),
       width=9, height=3, units='in', dpi=600)
